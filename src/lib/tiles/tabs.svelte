...
<script lang="ts">
	...
	const actions = $derived(
		(tile.actions !== undefined && tabsCtx?.actions?.get(tile.actions)) || undefined
	);
	...
	class DroppableSpacer extends TabsDroppable {
		protected onDrop(tabs: Tiles['tabs']): void {
			insert(tile, tile.children.length, tabs);
		}
	}
	...
	class DroppableRect extends TabsDroppable {
		...
		onMove(e: PointerEvent) {
			...
			Object.assign(this, getRectParts(rect, e.clientX, e.clientY, this.#edgeRatio));
		}
	}
